...
@click.option('--title', default="HHQN", help="Prefix of output files", type=str)
def run(seed, episodes, evaluation_episodes, batch_size, gamma, inverting_gradients, initial_memory_threshold,
        replay_memory_size, epsilon_steps, tau_actor, tau_actor_param , tau_actor_param_critic, use_ornstein_noise, learning_rate_actor,
        learning_rate_actor_param, learning_rate_actor_param_critic , epsilon_final, zero_index_gradients, initialise_params, scale_actions,
    ...
    from agents.hhqn import HHQNAgent
    ...
    if initialise_params:
        initial_weights = np.zeros((env.action_space.spaces[0].n, env.observation_space.spaces[0].shape[0]+3))
