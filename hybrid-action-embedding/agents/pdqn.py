...
# P-DDPG
    ...
    def step(self, state, action, reward, next_state, next_action, terminal, time_steps=1):
        act, all_action_parameters,action_em ,parameter_action_em = action
        self._step += 1
        ...
        self._add_sample(state, np.concatenate(([act],all_action_parameters,action_em,parameter_action_em)).ravel(), reward, next_state, np.concatenate(([next_action[0]],next_action[1])).ravel(), terminal=terminal)
        ...
        if not self.initial_phase:
            ...
            if self._step >= self.batch_size and self._step >= self.initial_memory_threshold:
                self.initial_phase_training(max_epochs=self.initial_phase_epochs)
    ...
    def save_models(self, prefix):
        ...
        torch.save(self.action_rep.state_dict(), prefix + '_embedding.pt')
    ...
    def load_models(self, prefix):
        ...
        self.action_rep.load_state_dict(torch.load(prefix + '_embedding.pt', map_location='cpu'))
    ...
    def self_supervised_update(self, s1, a1, a2, s2, reg=1):
        ...
        loss_act_rep = self.action_rep.unsupervised_loss(s1, a1.view(-1), a2, s2, normalized=True) * reg
    ...
    def initial_phase_training(self, max_epochs=1000,sup_batch_size=64):
        ...
        for counter in range(max_epochs):
            losses = []
            for states, actions, _, next_states, _ in self.replay_memory.batch_sample(batch_size=sup_batch_size, randomize=True):

                states = torch.from_numpy(states).to(self.device)  # 将numpy转换为torch
                actions_combined = torch.from_numpy(actions).to(self.device)  # make sure to separate actions and parameters
                d_actions = actions_combined[:, 0].long()
                actions = actions_combined[:, 1:4].long()
                action_parameters = actions_combined[:, 4:8]
                next_states = torch.from_numpy(next_states).to(self.device)

                loss = self.self_supervised_update(states, actions, action_parameters, next_states)
                losses.append(loss)
