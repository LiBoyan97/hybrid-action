...
from Src.Utils import ActionRepresentation
...
class PADDPGAgent(Agent):
    ...
    def __init__(self,
        ...
        self.target_action_rep=ActionRepresentation.Action_representation(state_dim=self.observation_space.shape[0],
                                                                     action_dim=self.num_actions,
                                                                     reduced_action_dim=self.num_actions,
                                                                     parameter_action_dim=self.action_parameter_size)
        ...
        if replay_memory is None:
            self.replay_memory = MemoryNStepReturns(replay_memory_size, observation_space.shape, (1+self.num_actions+self.action_parameter_size,), next_actions=False, n_step_returns=self.n_step_returns)
        else:
...
#act 返回的是动作、动作的emb、连续动作参数
    ...
    def act(self, state):
        with torch.no_grad():
            ...
            actions_emb += Variable(torch.from_numpy(noise1).type(float32), requires_grad=False)
    ...
    def step(self, state, action, reward, next_state, next_action, terminal, time_steps=1, optimise=True):
        ...
        self._add_sample(state, np.concatenate(([action],actions_emb.data, all_action_parameters.data)).ravel(), reward, next_state, terminal)
    ...
    def initial_phase_training(self, max_epochs=1000,sup_batch_size=64):
        ...
        for counter in range(max_epochs):
            losses = []
            states, actions, rewards, next_states, terminals = self.replay_memory.sample(sup_batch_size, random_machine=self.np_random)
            states = torch.from_numpy(states).to(device)
