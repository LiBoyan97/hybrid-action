...
class HHQNAgent(Agent):
    ...
    def __init__(self,
        ...
        #这块什么意思？？
        self.action_parameter_max_critic = torch.from_numpy(np.ones((self.num_actions,))).float().to(device)
        self.action_parameter_min_critic = -self.action_max.detach()
    ...
    def _optimize_td_loss(self):
        ...
        # Compute current Q-values using policy network
        q_values = self.actor_param_critic(states,actions.float(),action_parameters)
        y_predicted = q_values.gather(1, d_actions.view(-1, 1)).squeeze()
        y_expected = target
        loss_Q = self.loss_func(y_predicted, y_expected)
