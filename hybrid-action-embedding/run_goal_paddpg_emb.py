...
from agents.paddpg_emb import PADDPGAgent
...
def run(seed, episodes, evaluation_episodes, batch_size, gamma, inverting_gradients, initial_memory_threshold,
    ...
    for i in range(episodes):
        ...
        act, act_param, actions_emb, all_action_parameters = agent.act(state)
        action = pad_action(act, act_param)
        ...
        for j in range(max_steps):
            ...
            next_act, next_act_param, next_actions_emb, next_all_action_parameters = agent.act(next_state)
            next_action = pad_action(next_act, next_act_param)

            r = reward * reward_scale
            agent.step(state, (act, act_param, actions_emb, all_action_parameters), r, next_state,
                       (next_act, next_act_param, next_actions_emb, next_all_action_parameters), terminal,
                       optimise=True)

            act, act_param, actions_emb, all_action_parameters = next_act, next_act_param, next_actions_emb, next_all_action_parameters
            action = next_action
            state = next_state
...
def plot_reward(Reward):
    plt.plot(np.arange(len(Reward))*100, Reward, c='y', label='average_reward')
    plt.legend(loc='best')
    plt.ylabel('R')
    plt.xlabel('Epioside')
    plt.show()
...
def plot_p(possibility):
    plt.plot(np.arange(len(possibility))*100, possibility, c='y', label='win_possibility')
    plt.legend(loc='best')
    plt.ylabel('R')
    plt.xlabel('Epioside')
    plt.show()
