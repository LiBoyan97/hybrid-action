...
from agents.hhqn_1 import HHQNAgent
...
@click.option('--episodes', default=80000, help='Number of epsiodes.', type=int)
...
@click.option('--tau-actor-param-critic', default=0.1, help='Soft target network update averaging factor.', type=float)
...
@click.option('--learning-rate-actor-param-critic', default=0.001, help="Critic network learning rate.", type=float)
...
def run(seed, episodes, evaluation_episodes, batch_size, gamma, inverting_gradients, initial_memory_threshold,
        replay_memory_size, epsilon_steps, epsilon_final, tau_actor, tau_actor_param, tau_actor_param_critic,use_ornstein_noise,
        learning_rate_actor, learning_rate_actor_param,learning_rate_actor_param_critic, reward_scale, clip_grad, title, scale_actions,
...
def plot_reward(Reward):
    plt.plot(np.arange(len(Reward)), Reward, c='y', label='average_reward')
    plt.legend(loc='best')
    plt.ylabel('R')
    plt.xlabel('Epioside')
    plt.show()

def plot_p(possibility):
    plt.plot(np.arange(len(possibility)), possibility, c='y', label='win_possibility')
    plt.legend(loc='best')
    plt.ylabel('R')
    plt.xlabel('Epioside')
    plt.show()
