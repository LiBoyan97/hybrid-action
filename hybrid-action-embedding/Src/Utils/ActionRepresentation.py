...
class Action_representation(NeuralNet):
    def __init__(self,
        ...
        if true_embeddings:
            ...
            # pytorch都是有tensor计算的，而tensor里面的参数都是Variable的形式.tensor不能反向传播，variable可以反向传播。requires_grad=False不进行梯度学习
        else:
...
#a1 离散动作 a2 连续动作参数
    def unsupervised_loss(self, s1, a1,  s2, normalized=True):
